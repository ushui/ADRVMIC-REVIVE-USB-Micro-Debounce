...
1～15回まで設定可能で、デフォルト値は1回、推奨値は1～2回です。  
...
## マトリックス版について
ハードウェアリソースが足りずに実装できなかったため、通常版のみを公開しています。  
マトリックス版はありませんのでご了承ください（OSSなので神の降臨に期待）。  
...
こうして手元にあるハードウェアが壊れてしまい、前述のROM問題にも気を配りつつ実装しなければならないこともあり、断念した経緯があります。  
一致検出回数は3回に制限してロータリーエンコーダ機能を省いた仕様ですが、それでもそのような状況です。興味のある方はどうぞ（中にhexファイルが入っていますが、壊れてもよい方以外は書き込まないでください）。  
